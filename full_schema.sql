...
--
-- Name: adjust_wallet_balance(uuid, numeric); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.adjust_wallet_balance(user_id uuid, amount numeric) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  UPDATE users
...
ALTER FUNCTION public.adjust_wallet_balance(user_id uuid, amount numeric) OWNER TO postgres;

--
-- Name: complete_game_transaction(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.complete_game_transaction(game_id_input integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
declare
...
begin
  ...
  set is_winner = true
  where game_id = game_id_input and user_id = winner_record.user_id;

  -- 3. Payout to winner: 1.5x
  update users
  set wallet_balance = wallet_balance + (winner_record.amount_paid * 1.5)
  where id = winner_record.user_id;

  -- 4. Payout to losers: 0.8x
  for loser_record in
    select * from game_participants
    where game_id = game_id_input and user_id != winner_record.user_id
  loop
    update users
    set wallet_balance = wallet_balance + (loser_record.amount_paid * 0.8)
...
ALTER FUNCTION public.complete_game_transaction(game_id_input integer) OWNER TO postgres;

--
-- Name: ensure_user_row(uuid, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.ensure_user_row(uid uuid, uemail text) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  INSERT INTO users (id, email, wallet_balance)
  VALUES (uid, uemail, 500)
  ON CONFLICT (id) DO NOTHING;
END;
$$;


ALTER FUNCTION public.ensure_user_row(uid uuid, uemail text) OWNER TO postgres;
...
CREATE TABLE public.games (
    ...
    entry_fee numeric DEFAULT 100 NOT NULL,
    max_players integer DEFAULT 10 NOT NULL,
    current_players integer DEFAULT 0 NOT NULL,
    winner_id uuid,
    created_at timestamp with time zone DEFAULT now(),
    completed_at timestamp with time zone,
    CONSTRAINT games_status_check CHECK ((status = ANY (ARRAY['waiting'::text, 'completed'::text])))
);
...
--
-- Name: get_available_games(uuid); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.get_available_games(uid uuid) RETURNS SETOF public.games
    LANGUAGE sql STABLE
    AS $$
  SELECT g.*
  FROM games g
  WHERE g.status = 'waiting'
    AND NOT EXISTS (
      ...
      WHERE p.game_id = g.id AND p.user_id = uid
    );
$$;


ALTER FUNCTION public.get_available_games(uid uuid) OWNER TO postgres;
...
BEGIN
  INSERT INTO users (id, email, wallet_balance)
  VALUES (NEW.id, NEW.email, 500) -- start with ₹500
  ON CONFLICT (id) DO NOTHING;
  RETURN NEW;
END;
$$;
...
--
-- Name: increment_player_count(uuid); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.increment_player_count(p_game_id uuid) RETURNS public.games
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  g games;
BEGIN
  ...
  IF g.current_players >= g.max_players THEN
...
ALTER FUNCTION public.increment_player_count(p_game_id uuid) OWNER TO postgres;

--
-- Name: increment_players(uuid); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.increment_players(game_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
begin
  update games
  set current_players = current_players + 1
  where id = game_id and current_players < max_players;
end;
$$;
...
--
-- Name: increment_wallet(uuid, numeric); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.increment_wallet(uid uuid, amount numeric) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ALTER FUNCTION public.increment_wallet(uid uuid, amount numeric) OWNER TO postgres;

--
-- Name: join_game(uuid, uuid, numeric); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric) RETURNS void
    LANGUAGE plpgsql
    AS $$
begin
  -- Deduct wallet
  update users set wallet_balance = wallet_balance - p_entry_fee where id = p_user_id;

  -- Add participant
  insert into game_participants (game_id, user_id, amount_paid)
  values (p_game_id, p_user_id, p_entry_fee)
  on conflict (game_id, user_id) do nothing;

  -- Increment current_players
  update games set current_players = current_players + 1 where id = p_game_id;
end;
$$;


ALTER FUNCTION public.join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric) OWNER TO postgres;

--
-- Name: join_game_transaction(integer, uuid, numeric); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric) RETURNS void
    LANGUAGE plpgsql
    AS $$
begin
  -- Deduct wallet balance
  update users
  set wallet_balance = wallet_balance - entry_fee_input
  where id = user_id_input;

  -- Insert participant
  insert into game_participants (game_id, user_id, amount_paid, is_winner)
  values (game_id_input, user_id_input, entry_fee_input, false);
...
ALTER FUNCTION public.join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric) OWNER TO postgres;
...
CREATE TABLE public.game_participants (
    ...
    user_id uuid NOT NULL,
    amount_paid numeric DEFAULT 100 NOT NULL,
...
CREATE TABLE public.users (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT users_wallet_balance_check CHECK ((wallet_balance >= (0)::numeric))
);
...
--
-- Name: game_participants game_participants_game_id_user_id_key; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: idx_game_participants_game_id; Type: INDEX; Schema: public; Owner: postgres
--
...
--
-- Name: idx_game_participants_user_id; Type: INDEX; Schema: public; Owner: postgres
--
...
--
-- Name: idx_games_created_at; Type: INDEX; Schema: public; Owner: postgres
--
...
--
-- Name: users Allow read/write for logged in user; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Allow read/write for logged in user" ON public.users USING ((auth.uid() = id)) WITH CHECK ((auth.uid() = id));


--
-- Name: game_participants Anyone can read game participants; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Anyone can read game participants" ON public.game_participants FOR SELECT TO authenticated USING (true);


--
-- Name: games Anyone can read games; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Anyone can read games" ON public.games FOR SELECT TO authenticated USING (true);


--
-- Name: games Users can create games; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can create games" ON public.games FOR INSERT TO authenticated WITH CHECK (true);
...
--
-- Name: game_participants Users can read own participation; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can read own participation" ON public.game_participants FOR SELECT TO authenticated USING ((auth.uid() = user_id));
...
--
-- Name: game_participants; Type: ROW SECURITY; Schema: public; Owner: postgres
--
...
--
-- Name: FUNCTION adjust_wallet_balance(user_id uuid, amount numeric); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.adjust_wallet_balance(user_id uuid, amount numeric) TO anon;
GRANT ALL ON FUNCTION public.adjust_wallet_balance(user_id uuid, amount numeric) TO authenticated;
GRANT ALL ON FUNCTION public.adjust_wallet_balance(user_id uuid, amount numeric) TO service_role;


--
-- Name: FUNCTION complete_game_transaction(game_id_input integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.complete_game_transaction(game_id_input integer) TO anon;
GRANT ALL ON FUNCTION public.complete_game_transaction(game_id_input integer) TO authenticated;
GRANT ALL ON FUNCTION public.complete_game_transaction(game_id_input integer) TO service_role;


--
-- Name: FUNCTION ensure_user_row(uid uuid, uemail text); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.ensure_user_row(uid uuid, uemail text) TO anon;
GRANT ALL ON FUNCTION public.ensure_user_row(uid uuid, uemail text) TO authenticated;
GRANT ALL ON FUNCTION public.ensure_user_row(uid uuid, uemail text) TO service_role;
...
--
-- Name: FUNCTION get_available_games(uid uuid); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.get_available_games(uid uuid) TO anon;
GRANT ALL ON FUNCTION public.get_available_games(uid uuid) TO authenticated;
GRANT ALL ON FUNCTION public.get_available_games(uid uuid) TO service_role;
...
--
-- Name: FUNCTION increment_player_count(p_game_id uuid); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.increment_player_count(p_game_id uuid) TO anon;
GRANT ALL ON FUNCTION public.increment_player_count(p_game_id uuid) TO authenticated;
GRANT ALL ON FUNCTION public.increment_player_count(p_game_id uuid) TO service_role;


--
-- Name: FUNCTION increment_players(game_id uuid); Type: ACL; Schema: public; Owner: postgres
--
...
--
-- Name: FUNCTION increment_wallet(uid uuid, amount numeric); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.increment_wallet(uid uuid, amount numeric) TO anon;
GRANT ALL ON FUNCTION public.increment_wallet(uid uuid, amount numeric) TO authenticated;
GRANT ALL ON FUNCTION public.increment_wallet(uid uuid, amount numeric) TO service_role;


--
-- Name: FUNCTION join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric) TO anon;
GRANT ALL ON FUNCTION public.join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric) TO authenticated;
GRANT ALL ON FUNCTION public.join_game(p_user_id uuid, p_game_id uuid, p_entry_fee numeric) TO service_role;


--
-- Name: FUNCTION join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric) TO anon;
GRANT ALL ON FUNCTION public.join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric) TO authenticated;
GRANT ALL ON FUNCTION public.join_game_transaction(game_id_input integer, user_id_input uuid, entry_fee_input numeric) TO service_role;
...
--
-- Name: TABLE game_participants; Type: ACL; Schema: public; Owner: postgres
--
